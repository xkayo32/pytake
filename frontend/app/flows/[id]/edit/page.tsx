    ...
    const loadFlowById = async () => {
      try {
        ...
        if (sessionFlow) {
          ...
          // Extrair nodes e edges do campo flow se existir
          const flowNodes = flowData.flow?.nodes || flowData.nodes || []
          const flowEdges = flowData.flow?.edges || flowData.edges || []
          
          const correctedNodes = flowNodes.map((node: any) => {
        ...
        // Try to load from backend API - buscar na lista já que não há rota individual
        const response = await fetch('/api/v1/flows')
        if (response.ok) {
          const allFlows = await response.json()
          const flowsArray = allFlows.flows || allFlows // Backend pode retornar {flows: [...]} ou diretamente [...]
          const flowData = flowsArray.find((f: any) => f.id === flowId)
  ...
  const handleActivateFlow = useCallback(async () => {
    ...
    try {
      ...
      if (!currentFlowId || currentFlowId.startsWith('flow-')) {
        ...
        if (!currentFlowId) {
          console.error('❌ Resposta do backend sem ID:', createdFlow)
          throw new Error('Backend não retornou ID do flow criado')
        }
        
        console.log('✅ Flow criado com ID:', currentFlowId)
      ...
      console.log('📡 Criando flow ativo:', {
      ...
      const createResponse = await fetch('/api/v1/flows', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(activeFlowData)
      })
      ...
      if (!createResponse.ok) {
        ...
        throw new Error(`Erro ao criar flow ativo: ${errorMessage}`)
      }
