...
export default function ChatbotBuilderPage() {
  ...
  const handleAddNode = (nodeType: any) => {
    ...
    const newNode: Node = {
      ...
      data: {
        ...
        color: COLOR_MAP[nodeType.color],
      },
    };
  ...
  // Handler para deletar nós com Delete/Backspace
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    if ((event.key === 'Delete' || event.key === 'Backspace') && selectedNode) {
      // Não deletar se estiver editando um input
            ...
            <Panel position="top-right" className="bg-white dark:bg-gray-800 px-3 py-2 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-1.5 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
                  title={showNodesPanel ? 'Ocultar Painel de Nós' : 'Mostrar Painel de Nós'}
                >
                  {showNodesPanel ? <PanelLeftClose className="w-4 h-4" /> : <PanelLeftOpen className="w-4 h-4" />}
                </button>
                <button
                  onClick={() => setShowPropertiesPanel(!showPropertiesPanel)}
                  className="p-1.5 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
                  title={showPropertiesPanel ? 'Ocultar Painel de Propriedades' : 'Mostrar Painel de Propriedades'}
                >
                  {showPropertiesPanel ? <PanelRightClose className="w-4 h-4" /> : <PanelRightOpen className="w-4 h-4" />}
                </button>
